...
extends ReporterCommand {
    ...
    private static final List<Usage> usages = Collections.unmodifiableList(ArrayUtil.arrayToArrayList(new Usage[]{new Usage((Entry<String>)DowngradePhrases.downgradeHelp, (Entry<String>)DowngradePhrases.downgradeHelpDetails)}));
    ...
    public void execute(CommandSender sender, ArrayList<String> args) throws NoLastViewedReportException, IndexOutOfRangeException, IndexNotANumberException {
        try {
            ...
            ModLevel newPriority = this.getNextLowestPriorityLevel(index);
            if (newPriority == ModLevel.UNKNOWN) {
                String output = this.getManager().getLocale().getString(DowngradePhrases.reportIsAtLowestPriority);
            ...
            log.error("Failed to downgrade report priority!", (Throwable)e);
